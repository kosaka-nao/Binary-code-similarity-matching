...
with open('./ffmpeg_dataset.pickle', 'rb') as handle:
    ffmpeg_dataset = pickle.load(handle)

db_func = list(ffmpeg_dataset.keys())

train_func,test_func = train_test_split(db_func,test_size=0.2, random_state=42)

#train_func = train_func[:300]
#test_func = test_func[:100]
...
def run():
    ...
    # minutes.
    config['training']['n_training_steps'] = 500000 #50000
